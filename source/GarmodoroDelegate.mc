...
function isLongBreak() {
  return (pomodoroNumber %
          App.getApp().getProperty("numberOfPomodorosBeforeLongBreak")) == 0;
}
...
class GarmodoroDelegate extends Ui.BehaviorDelegate {
  function initialize() {
    ...
    // timer.start( method(), 60 * 1000, true );
  }
  ...
  function pomodoroCallback() {
    ...
    if (minutes == 0) {
      ...
      minutes = App.getApp().getProperty(isLongBreak() ? "longBreakLength"
                                                       : "shortBreakLength");
  ...
  function shouldTick() { return App.getApp().getProperty("tickStrength") > 0; }

  function tickCallback() {
    ping(App.getApp().getProperty("tickStrength"),
         App.getApp().getProperty("tickDuration"));
  }
  ...
  function onSelect() {
    if (isBreakTimerStarted || isPomodoroTimerStarted) {
      Ui.pushView(new Rez.Menus.StopMenu(), new StopMenuDelegate(),
                  Ui.SLIDE_UP);
      return true;
    }
    ...
    if (me.shouldTick()) {
      tickTimer.start(method(
                          : tickCallback),
                      App.getApp().getProperty("tickFrequency") * 1000, true);
    }
